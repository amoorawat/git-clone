...
class Commit(GitObject):
    ...
    def _serialize_commit(self):
        ...
        lines.append(f"author {self.committer} {self.timestamp} +0000")
        lines.append("")
    ...
    def from_content(cls, content: bytes) -> Commit:
        ...
        commit = cls(tree_hash, parent_hashes, author, committer, message, timestamps)
...
class Repository:
    ...
    def init(self) -> bool:
        ...
        # create initial HEAD pointing to a branching
    ...
    def add_file(self, path: str):
        # first create a full path again.
        ...
        blob = Blob(content)
        # store the blob objects in database (.git/objects)
        blob_hash = self.store_object(blob)
        # update the index to inlcude the file
    ...
    def add_directory(self, path: str):
        ...
        for file_path in full_path.rglob("*"):
            ...
            if file_path.is_file():
                # dont' want to include .pygit folder
            ...
            if added_count > 0:
                ...
                print(f"Diretory {path} already up to date")
    ...
    def commit(self, message: str, author: str = "PyGit User <user@pygit.com>"):
        # create a tree object from the index (stagging area)
        ...
        index = self.load_index()
        if not index:
            print("nothin to commit, working tree clean")
            return None
    ...
    def checkout(self, branch: str, create_branch: bool):
        # compute the files to clear from the previous branch
        ...
        if not branch_file.exists():
            if create_branch:
                ...
                print("Use 'python main.py checkout -b {branch}' to create and switch the branch")
                return
    ...
    def restore_working_directory(self, branch: str, files_to_clear: set[str]):
        ...
        if target_commit.tree_hash:
            ...
            # Need to add logic -- if user does not commit and trying to move to other branch give error.
        self.save_index({})
    ...
    def branch(self, branch_name: str, delete: bool = False):
        if delete and branch_name:
            ...
            if branch_file.exists():
                branch_file.unlink()
                print(f"Delted the branch {branch_name}")
            else:
    ...
    def build_index_from_treeHash(self, tree_hash: str, prefix: str = ""):
        index = {}
        try:
            tree_object = self.load_object(tree_hash)
            tree = Tree.from_content(tree_object.content)
            ...
            for mode, name, obj_hash in tree.entries:
                ...
                if mode.startswith("100"):
                    ...
                    subindex = self.build_index_from_treeHash(obj_hash, f"{full_name}/")
    ...
    def status(self):
        ...
        if current_commit_hash:
            try:
                ...
                commit = Commit.from_content(commit_obj.content)
                if commit.tree_hash:
                    last_index_file = self.build_index_from_treeHash(commit.tree_hash)
            except:
                last_index_file = {}

        # figure out all the files present within the working directory
        working_files = {}   # file_name -> hash
        ...
        # what files have modified but not stagged
...
def main():
    ...
    branch_parser.add_argument("name", nargs="?", help="Create a new branch")
    ...
    # status parser
    status_parser = subparsers.add_parser("status", help="show all repository")
